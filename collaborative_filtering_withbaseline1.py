    ...
    if mode=='user':
        ...
        for idx,row in test.iterrows(): #########remove head
            ...
            if rating<=0:
                ...
                no_of_zero+=1   
            #print 'Truth:', row['rating'], 'Pred:', rating, " ",no_of_zero, " ",no_of_ratings
...
print 'User-User-> Total time:', time()- start_time_item
print 'User-User-> RMSE:', evaluation.RMSE(predictions_usr, truth[0:10])
print 'spearman_rank_correlation', evaluation.spearman_rank_correlation(predictions_usr, truth[0:10])
print 'Precision on top K' , evaluation.top_k_precision(predictions_usr, test.head(10), means_mat, user_map)
...
predictions=predict_baseline(train.T, item_dist, test, user_map, movie_map, 10,'item',temp2.T,means_mat,movie_mat)
predictions=np.squeeze(predictions)
print 'Item-item-> Total time:', time()- start_time_item
print 'Item-item-> RMSE:', evaluation.RMSE(predictions, truth[0:10])
print 'spearman_rank_correlation', evaluation.spearman_rank_correlation(predictions, truth[0:10])
print 'Precision on top K' , evaluation.top_k_precision(predictions_usr, test.head(10), movie_mat, movie_map, 5, False)
